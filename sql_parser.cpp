...
bool executeInsert(const string& sql_query) {
    ...
    string schema_name = schema_data.value("name", ""); // Получаем имя схемы
    string locked_file_path = schema_name + "/" + table_name + "/locked";
    ...
    lock_file.close();
    cout << "Содержимое файла блокировки перед записью: " << lock_status << endl;

    if (lock_status == "1") {
        cerr << "Ошибка: таблица " << table_name << " заблокирована." << endl;
        return false;
    }
    ...
    if (!lock_file_write.is_open())
    {
        std::cerr << "failed to open file for writing! " << locked_file_path << std::endl;
        return false;
    }

    ifstream lock_file2(locked_file_path);


    lock_file_write << "1" << std::endl;
    lock_file2 >> lock_status;
    lock_file_write.close();
    lock_file2.close();

        cout << "Содержимое файла блокировки после записи: " << lock_status << endl;

    // --- КОНЕЦ БЛОКИРОВКИ --- 
    cout << "Таблица " << table_name << " заблокирована!" << endl;
    ...
    if (!unlock_file.is_open()) {
        cerr << "Ошибка: не удалось открыть файл блокировки для записи: " << locked_file_path << endl;
        return false;
    }
    unlock_file << "0" << std::endl;
    unlock_file.close();
    // --- КОНЕЦ РАЗБЛОКИРОВКИ ---
